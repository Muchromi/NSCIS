<?php
    ...
    $debit = $description = $date = $member_id ='';
    $debiterr = $descriptionerr = $dateerr = $member_iderr ='';
    ...
    if (isset($_POST['submit'])) {
      ...
      function changeRupiah($debit){
        $result = preg_replace("/[^0-9]/", "", $debit);
        return $result;
      }
      ...
      // validate rupiah
      if (empty(trim($debit))) {
        $debitterr = "please fill in the credit"; 
      }else{
        $debit_result = changeRupiah($debit);
      }
      ...
      while ($saldo = mysqli_fetch_array($last_query)){
          $result_saldo = $saldo['saldo'] + $debit_result = changeRupiah($debit);
      }

      //insert to table
      if (!empty($date) && !empty($debit_result) && !empty($description)) {
        $sql = "INSERT INTO pattycash (date, debit, description, saldo, member_id) VALUES ('{$date}', '{$debit_result}', '{$description}', '{$result_saldo}', '{$member_id}')";
...
<div class="row">
      ...
      <div class="column middle">
          ...
          <div class="container">
            <form action="add-debit.php" method="post" enctype="multipart/form-data">
...
<!-- member -->
            ...
            <div class="row">
              ...
              <div class="row">
                ...
                <div class="col-75">
                  <input type="text" id="dengan-rupiah"  value="<?php echo $debit; ?>" name="debit" placeholder="debit..">
                  <span class="error"><?php echo $debiterr; ?></span>
                </div>
              </div>
